...
def perfil(request):
    if request.method == "GET":
        ...
        if usuario.exists():
            messages.add_message(request, constants.ERROR, 'Esse usúario já existe.')
            ...
            try:
               ...
               messages.add_message(request, constants.SUCCESS, 'Todos os dados alterados com sucesso.') 
               return redirect('/jobs/perfil/')
           
            except:
                messages.add_message(request, constants.ERROR, 'Ero interno do sistema, contate um administrador.') 
                return redirect('/jobs/perfil/')
@login_required(login_url='/auth/login')         
